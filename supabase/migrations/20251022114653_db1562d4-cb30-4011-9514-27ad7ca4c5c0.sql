-- Create admin-only payment transactions table for Stripe payment data
...
DROP POLICY IF EXISTS "Only admins can access payment transactions" ON public.payment_transactions;
...
-- Add validation trigger for lesson_recordings to prevent teacher/student ID manipulation
...
-- Create helper functions for users to check payment status without exposing Stripe IDs
