...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'payment_transactions') THEN
    ...
    -- Migrate existing payment data if columns still exist
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tour_bookings' AND column_name='stripe_payment_intent_id') THEN
