...
export const PersonalizedTourWorkflow: React.FC<PersonalizedTourWorkflowProps> = ({
  ...
  const handleSuggestionToggle = (suggestionId: string) => {
    setSelectedSuggestions(prev => prev.includes(suggestionId) ? prev.filter(id => id !== suggestionId) : [...prev, suggestionId]);
  };
  ...
  const getMapMarkers = () => {
    return tourSuggestions.filter(s => selectedSuggestions.includes(s.id)).map(s => ({
      id: s.id,
      position: s.location,
      title: s.name,
      type: s.type,
  ...
  return <div className="space-y-6">
      {step === 'suggestions' && <div className="space-y-6">
          <Card>
            <CardHeader>
            ...
            <CardContent>
              ...
              {selectedSuggestions.length > 0 && <div className="mb-6">
              ...
              <div className="grid gap-4">
                {tourSuggestions.map(suggestion => <Card key={suggestion.id} className={`border-2 transition-all ${selectedSuggestions.includes(suggestion.id) ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`}>
                    <CardContent className="p-4">
                      <div className="flex items-start gap-4">
                        <Checkbox checked={selectedSuggestions.includes(suggestion.id)} onCheckedChange={() => handleSuggestionToggle(suggestion.id)} className="mt-1" />
              ...
              <div className="mt-6 flex justify-between items-center">
                ...
                <Button onClick={handleNextStep} disabled={selectedSuggestions.length === 0} className="bg-gradient-wanderlust hover:opacity-90 bg-cyan-600 hover:bg-cyan-500">
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                {tourSuggestions.filter(s => selectedSuggestions.includes(s.id)).map(suggestion => <div key={suggestion.id} className="flex justify-between items-center py-2 border-b">
              ...
              <div className="pt-4 border-t">
                <div className="flex justify-between font-semibold">
                  ...
                  <span>
                    ${tourSuggestions.filter(s => selectedSuggestions.includes(s.id)).reduce((sum, item) => sum + item.price, 0) + (selectedGuide ? tourGuides.find(g => g.id === selectedGuide)?.dailyRate || 0 : 0)}
                  </span>
                </div>
              </div>
      ...
      {step === 'self-explore' && <Card>
          ...
          <CardContent>
            ...
            <div className="space-y-2">
              {tourSuggestions.filter(s => selectedSuggestions.includes(s.id)).map(suggestion => <div key={suggestion.id} className="flex items-center gap-2">
